...
void main()
{
    ...
    vec4 pixel_color = texture2D(currentTexture, uv);
    ...
    vec3 lightColor1 = vec3(1,1,0);
    
    vec2 light2 = vec2(sin(iTime+3.0)*-2.0,cos(iTime+7.0)*1.0)*0.2+center;
    vec3 lightColor2 = vec3(0.23,1,0.23);
    
    vec2 light3 = vec2(sin(iTime+3.0)*2.0,cos(iTime+14.0)*-1.0)*0.2+center;
    vec3 lightColor3 = vec3(1,1,0);

    // vec2 light4 = vec2(sin(iTime+9.0 + 20.0)*2.0,cos(iTime+16.0)*-1.0)*0.2+center;
    // vec3 lightColor4 = vec3(0.1, 0.1, 1.0);
    
    float cloudIntensity1 = 0.5*(0.5 -(4*distance(uv, light1)));
    float lighIntensity1 = 1.0/(1000.0*distance(uv,light1));

    float cloudIntensity2 = 0.5*(0.5 -(4*distance(uv, light2)));
    float lighIntensity2 = 1.0/(1000.0*distance(uv,light2));
    
    float cloudIntensity3 = 0.5*(0.5 -(4*distance(uv, light3)));
    float lighIntensity3 = 1.0/(1000.0*distance(uv,light3));

    // float cloudIntensity4 = 0.5*(2.0 -(3*distance(uv, light3)));
    // float lighIntensity4 = 1.0/(400.0*distance(uv,light3));

	  gl_FragColor = vec4(
                  vec3(cloudIntensity1*clouds(uv))*lightColor1 + lighIntensity1*lightColor1 +
